...
It does, on every quantity measured. Over increases of comparable size,
raising $k$ drives divergence to zero and cuts interquartile width by
42--66\% in all six series, while raising $n$ moves neither. Two of the
four quantities never involve the synthesizer at all, so what the
threshold describes is the stratum rather than \texttt{synthpop}.
...
6--8\% power to detect them, which is what the sweeps returned, so at these
sizes no count reachable inside a subsample makes the estimate usable.
...
rows; all subsampling draws from this pool. Age is MIMIC-IV's
...
subsample estimate below is read against it. The mechanism was first
checked on a simulation with a known data-generating process, where bias
against the truth is measurable directly; that notebook is in the
repository and only the MIMIC-IV results are reported here. The three interactions differ
...
All six curves reach zero at $k = 80$ and stay there. That zero is 0 of 100
replicates, which bounds the rate at 3.6\% rather than establishing it as
nil, and the grid steps from 40 to 80, so what the data supports is a
threshold in $(40, 80]$. Wherever it sits, it moves with neither
...
failure threshold common to all six series.
...
misses significance is not evidence that nothing is there. Divergence is identified by the size of the coefficient rather than by a
separation test, and the penalized fit that would distinguish separation
from thin information \cite{firth1993} was not run, so the threshold is
described here and not explained. One continuous variable and one outcome
were examined, Renal Failure and Hypertension were screened but not
analyzed, and all results come from a single MIMIC-IV snapshot.

% -------------------------------------------------------
\section{Conclusion}

The positive count governs every quantity measured here, and sample size
governs none of them. Divergence falls to zero by a count of eighty in
